...
function hline(doc, x1, x2, y, lw = 0.3, useBrown = true) {
  if (useBrown) dBrown2(doc);
  else doc.setDrawColor(...BLK);
  doc.setLineWidth(lw);
  doc.line(x1, y, x2, y);
}
...
function imgBox(doc, data, bx, by, bw, bh) {
  ...
  if (!data) {
    ...
    doc.setFontSize(7);
    tGrey(doc);
    doc.text("No image available", bx + bw / 2, by + bh / 2 + 2, {
      align: "center",
    });
    return;
  }
  try {
    ...
    doc.setFontSize(7);
    tGrey(doc);
    doc.text("Image unavailable", bx + bw / 2, by + bh / 2 + 2, {
      align: "center",
    });
  }
}
...
function buildCover(doc, logoData, reportId, dateStr, timeStr) {
  ...
  tGrey(doc);
  doc.text(
    "Forensic Document Examination System",
    logoX + logoW + 4,
    logoY + 18,
  );
  ...
  doc.setFontSize(10);
  tGrey(doc);
  doc.text("AI-Powered Signature & Document Examination", cx, y, {
    align: "center",
  });
  y += 22;
  ...
  doc.setFontSize(8);
  tGrey(doc);
  doc.text("TRUSTED EVIDENCE.   CLEAR INSIGHT.", cx, PH - 26, {
    align: "center",
  });
  doc.setFont("Outfit", "normal");
  doc.setFontSize(7);
  tGrey(doc);
  doc.text("DocLens AI Forensic Engine  |  Version 1.0", cx, PH - 20, {
    align: "center",
  });
}
...
function buildPage1(doc, D, logoData) {
  doc.addPage();
  const onNew = (d) =>
    drawHeader(d, logoData, D.reportId, D.dateStr, D.timeStr, "— of —");
  ...
  const metrics = [
    {
      lbl: "VERDICT",
      val: D.verdictIsForged ? "Forgery Suspected" : "Genuine",
    },
    { lbl: "CONFIDENCE", val: `${D.confidence ?? "—"}%` },
    {
      lbl: "OVERALL SIMILARITY",
      val: `${D.bestScore ?? D.confidence ?? "—"}%`,
    },
  ...
  const leftCI = [
    ...
    [
      "File Size",
      D.caseData?.questionedFile?.size
  ...
  tBlack(doc);
  doc.text(
    "Signature region automatically located within the submitted document using CNN-based detection.",
    ML,
    y,
  );
  y += 9;
...
function buildPage2(doc, D, logoData, qCropData, refCropData, heatmapData) {
  ...
  const match = (keys) =>
    kd.find((k) => keys.some((kw) => k.label?.toLowerCase().includes(kw)));

  const featureRows = [
    [
      "Allograph Morphology",
      match(["stroke", "letter", "proportion"])
        ? "Class & individual characteristics diverge from reference set"
        : "Class characteristics consistent with reference exemplars",
    ],
    [
      "Axial Pen Pressure Distribution",
      match(["pressure"])
        ? "Lateral force variation pattern inconsistent with reference"
        : "Lateral force distribution within expected range",
    ],
    [
      "Baseline Trajectory",
      match(["baseline"])
        ? "Deviation from habitual writing line; possible feigned tilt"
        : "Baseline trajectory consistent with reference specimens",
    ],
    [
      "Pen-Lift Sequencing",
      match(["pen lift", "pen-lift"])
        ? "Lift positions atypical; suggests interrupted execution"
        : "Pen-lift positions consistent with reference sequence",
    ],
    [
      "Entry & Approach Strokes",
      match(["entry"])
        ? "Initial stroke formation inconsistent; possible hesitation"
        : "Approach strokes consistent with habitual motor pattern",
    ],
    [
      "Terminal & Exit Strokes",
      match(["exit"])
        ? "Exit stroke truncated or retraced; inconsistent formation"
        : "Exit strokes consistent with reference terminal strokes",
    ],
    [
      "Fluency & Line Quality",
      match(["stroke", "curvature"])
        ? "Line quality disrupted; tremor or retouching artifacts present"
        : "Line quality consistent with fluent, habitual execution",
    ],
    [
      "Axial Slant Angle",
      match(["slant"])
        ? "Writing slant deviates beyond ±5° of reference mean"
        : "Slant angle within ±5° tolerance of reference specimens",
    ],
    [
      "Spatial Proportionality",
      match(["proportion", "letter", "spacing"])
        ? "Inter-letter spacing and height ratios show significant shift"
        : "Spatial proportions consistent with habitual writing habit",
    ],
    [
      "Relative Connecting Strokes",
      match(["curvature"])
        ? "Connecting arc geometry and curvature deviate from reference"
        : "Connecting strokes consistent with reference arc geometry",
    ],
    [
      "Speed & Rhythm Indicators",
      D.verdictIsForged
        ? "Slow, laboured execution; speed indicators inconsistent"
        : "Writing speed and rhythm consistent with natural execution",
    ],
    [
      "Retrace & Correction Artifacts",
      D.verdictIsForged
        ? "Retrace artifacts and patch-corrections detected"
        : "No retrace or correction artifacts observed",
    ],
  ];
...
function buildPage3(doc, D, logoData, heatmapData, qCropData) {
  ...
  const simRows =
    realFeatureScores.length > 0
      ? [
          ["Overall Cosine Similarity", best],
          ...realFeatureScores.map((f) => [f.label, f.score]),
        ]
      : [["Overall Cosine Similarity", best]];
...
function buildPage4(doc, D, logoData) {
  ...
  const techRows = [
    ...
    [
      "Feature Extractor",
      `${D.modelName} — pretrained on ImageNet, fine-tuned on signature corpus`,
    ],
    [
      "Embedding Dimensionality",
      "512-dimensional L2-normalised feature vector per specimen",
    ],
    [
      "Similarity Metric",
      "Cosine distance (1 − cos θ);  authenticity threshold ≥ 0.75",
    ],
    [
      "Region Detection",
      "CNN-based bounding-box detector; morphological post-processing",
    ],
    [
    ...
    [
      ...
      "Greyscale → adaptive binarisation → stroke normalisation → 224×224 resize",
    ],
    [
      "Reference Set Size",
      `${D.refCount} specimen${D.refCount !== 1 ? "s" : ""} submitted`,
    ],
    ["Processing Time", D.analysisTime || "—"],
    [
      "Standards Reference",
      "SWGDOC Standard for Examination of Handwritten Items; OSFE Guidelines",
    ],
  ];
...
export async function generateReport(caseData) {
  ...
  const reportId = `DCL-${year}-${String(caseId)
    .replace(/[^0-9]/g, "")
    .slice(0, 6)
    .padStart(6, "0")}`;
