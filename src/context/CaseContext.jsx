...
export function CaseProvider({ children }) {
  ...
  const setReferenceFiles = useCallback((files) => {
    ...
    setCaseData((prev) => ({
      ...prev,
      referenceFiles: arr,
      referenceFileURLs: urls,
    }));
  }, []);

  const setCaseInfo = useCallback(
    ({ caseRef, caseType, subjectNames, signerName, dateReceived }) => {
      setCaseData((prev) => ({
        ...prev,
        caseRef,
        caseType,
  ...
  const setStorageURLs = useCallback(
    ({ questionedStorageURL, referenceStorageURLs }) => {
      setCaseData((prev) => ({
        ...prev,
        questionedStorageURL,
        referenceStorageURLs,
      }));
    },
    [],
