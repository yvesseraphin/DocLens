...
export function ChangePasswordPage({ navigate }) {
  ...
  async function handleSubmit() {
    ...
    if (!accessToken) { toast.error('Reset token not found. Please request a new password reset link.'); return; }
    setLoading(true);
    try {
      await resetPassword({ access_token: accessToken, refresh_token: refreshToken, new_password: password });
  ...
  return (
    <main className="login-page">
      <section className="login-image-panel" aria-label="DocLens change password illustration" />
      <section className="login-form-panel">
        ...
        <form className="login-card" onSubmit={(e) => e.preventDefault()}>
          ...
          <button type="button" className="login-submit" disabled={loading} onClick={handleSubmit}>
