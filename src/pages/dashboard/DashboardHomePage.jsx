  ...
  return (
    <div className="db-donut" role="img" aria-label={`${value}% overall`}>
      <svg className="db-donut-svg" viewBox="0 0 160 160" aria-hidden="true">
        ...
        <circle className="db-donut-arc db-donut-arc-fast" cx="80" cy="80" r={radius}
          strokeDasharray={`${fastLength} ${circumference}`} strokeDashoffset={0} />
        <circle className="db-donut-arc db-donut-arc-slow" cx="80" cy="80" r={radius}
          strokeDasharray={`${slowLength} ${circumference}`} strokeDashoffset={-(fastLength + gap)} />
      </svg>
...
export function DashboardHomePage({ navigate }) {
  ...
  const apiStatus  = apiOnline === null ? 'Checking…' : apiOnline ? 'Online' : 'Offline';
  const healthRows = [
    { label: 'AI Connection',     status: apiStatus },
    { label: 'Image Recognition', status: apiOnline === null ? 'Checking…' : apiOnline ? 'Active' : 'Unavailable' },
    { label: 'File Storage',      status: apiOnline === null ? 'Checking…' : apiOnline ? 'Healthy' : 'Unavailable' },
    { label: 'Official Records',  status: apiOnline === null ? 'Checking…' : apiOnline ? 'Updated' : 'Unavailable' },
  ];

  const total    = cases.length;
  const flagged  = cases.filter(c => c.verdict === 'FORGED').length;
  const pending  = cases.filter(c => c.status === 'pending' || c.status === 'analyzing').length;
  ...
  const pageRows   = cases.slice((page - 1) * ROWS_PER_PAGE, page * ROWS_PER_PAGE);

  return (
    <section className="db-root" aria-labelledby="dashboard-title">
      <div className="db-hero">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <h1 className="db-title" id="dashboard-title">Dashboard</h1>
        </div>
      ...
      <div className="db-content">
        <section className="db-card db-recent-card" aria-labelledby="recent-title">
          <h2 className="db-card-title" id="recent-title">Recent Cases</h2>

          {loading ? (
            <div style={{ padding: '24px 0', display: 'flex', justifyContent: 'center' }}>
              <div className="add-spinner-ring" style={{ width: 36, height: 36, borderWidth: 3, marginBottom: 0 }} />
            </div>
          ) : cases.length === 0 ? (
            <p style={{ padding: '16px 0', color: '#888', fontSize: '0.9rem' }}>No cases yet.</p>
          ) : (
            <>
              <table className="db-table">
                <thead>
                  <tr><th>Case ID</th><th>Signer</th><th>Decision</th><th>Date</th></tr>
                </thead>
                <tbody>
                  {pageRows.map((c) => (
                    <tr key={c.id} style={{ cursor: 'pointer' }} onClick={() => navigate && navigate('/cases')}>
                      <td>{c.case_ref || c.id?.slice(0, 8)}</td>
                      <td>{c.signer_name || c.subject_names || '—'}</td>
                      <td>
                        {c.verdict ? (
                          <span className={`db-badge ${c.verdict === 'GENUINE' ? 'db-badge-verified' : 'db-badge-flagged'}`}>
                            {c.verdict === 'GENUINE' ? 'Verified' : 'Flagged'}
                          </span>
                        ) : (
                          <span className="db-badge" style={{ background: '#f0f0f0', color: '#666' }}>{c.status || 'Pending'}</span>
                        )}
                      </td>
              ...
              {totalPages > 1 && (
                <div className="db-pagination" aria-label="Pagination">
                  <button className="db-pg-btn" type="button" aria-label="Previous page" disabled={page === 1}
                    onClick={() => setPage(p => Math.max(1, p - 1))}>
                    <ChevronLeft size={13} />
                  </button>
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                    <button key={p} className={`db-pg-btn${page === p ? ' db-pg-active' : ''}`} type="button" onClick={() => setPage(p)}>{p}</button>
                  ))}
                  <button className="db-pg-btn" type="button" aria-label="Next page" disabled={page === totalPages}
        ...
        <aside className="db-card db-health-card" aria-labelledby="cloud-title">
          <h2 className="db-card-title" id="cloud-title">Cloud Health</h2>
